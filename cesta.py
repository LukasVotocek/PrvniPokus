class Mesto:
    def __init__ (self,nazev: str)->None:
        self.nazev=nazev
        self.sousede:list[tuple[str,int]]=[]
...
class Graph:
    ...
    def new_edge(self, from_city: str, to_city: str, distance: int) -> None:
        f_city=self._vrat_nebo_zaloz_mesto(from_city)
        t_city=self._vrat_nebo_zaloz_mesto(to_city)
        f_city.sousede.append((to_city,distance))
        t_city.sousede.append((from_city,distance))

    def _vrat_nebo_zaloz_mesto(self,nazev)->Mesto:
        mesto=self.mesta.get(nazev)
        if mesto is None:
            mesto=Mesto(nazev)
